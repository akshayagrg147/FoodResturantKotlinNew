...
class CartItemssAdapter(private var categories1: List<CartItems>, cartItemClickListnerr: cartItemClickListner, database1: ProductDatabase
    ...
    inner class PostViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView){
       fun onbind(cartItems: CartItems) {
           var cartItems:CartItems= database.contactDao().getProductBasedId(cartItems.ProductIdNumber)
           var totalNumber:Int= database.contactDao().getProductBasedIdCount(cartItems.ProductIdNumber)
           itemView.totalquantity.text=totalNumber.toString()
           itemView.priceCategory.text=(cartItems.let { it.strProductPrice?.times(totalNumber) }).toString()
